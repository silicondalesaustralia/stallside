...
export async function uploadSubscriptionOfferImage(
  standId: string,
  offerId: string,
  file: File,
): Promise<string> {
  ...
  const safeName =
    file.name.replace(/[^\w.\-]+/g, "_").slice(0, 80) || "subscription";
  const blob = await put(
    `stands/${standId}/subscription-offers/${offerId}/${Date.now()}-${safeName}`,
    file,

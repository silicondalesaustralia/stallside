/** Shared social network icons (24×24 path data). */
...
export type SocialLinkDef = {
  id: SocialNetwork;
  label: string;
  /** Form field name on branding. */
  field: string;
  path: string;
};

export const STAND_SOCIALS: SocialLinkDef[] = [
  {
    id: "instagram",
    label: "Instagram",
    field: "instagramUrl",
...
/** Normalize a pasted social/website URL; empty → null. Rejects javascript: etc. */
export function parseSocialUrl(raw: string): string | null | { error: string } {
  ...
  try {
    ...
    if (candidate.length > 300) {
      return { error: "Link is too long." };
    }
    return url.toString();
  } catch {
    return { error: "Enter a valid link (e.g. https://instagram.com/yourfarm)." };
  }
}

export function activeStandSocials(urls: StandSocialUrls) {
  return STAND_SOCIALS.filter((s) => Boolean(urls[s.field as keyof StandSocialUrls])).map(
    (s) => ({
      ...s,
      href: urls[s.field as keyof StandSocialUrls] as string,
    }),
  );
}

export function standSocialFromStand(stand: StandSocialUrls): StandSocialUrls {
  return {
